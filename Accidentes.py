...
data = pd.read_csv('accidentes.csv', header=0)


def accidente(data: pd.DataFrame) -> dict:
    # Conseguimos la cantidad total de Accidente de Gravedad
    valores = data['GRAVEDAD_ACCIDENTE'].value_counts().to_list()
    suma = 0
    ...
    accidenteMO = data['CLASE_ACCIDENTE'].value_counts().keys().tolist()
    val1 = accidenteMO[0]
    numAccidenteOC = data['CLASE_ACCIDENTE'].value_counts().tolist()
    ...
    valores3 = data['CANT_HERIDOS_EN _SITIO_ACCIDENTE'].max()
    part = data[['FECHA_ACCIDENTE', 'CANT_HERIDOS_EN _SITIO_ACCIDENTE']]
